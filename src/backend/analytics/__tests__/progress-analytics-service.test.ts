import { 
  KinesisProgressAnalyticsService, 
  AnalyticsEvent, 
  ProgressReport,
  ProgressTrend,
  LearningInsight
} from '../progress-analytics-service';
import { 
  ConversationSession, 
  SessionMetrics, 
  ProgressMetrics,
...
(PutRecordsCommand as jest.MockedClass<typeof PutRecordsCommand>).mockImplementation((input) => {
  const command = { input };
  mockPutRecordsCommand(input);
  return command as any;
});

(KinesisClient as jest.MockedClass<typeof KinesisClient>).mockImplementation(() => mockKinesisClient as any);

describe('KinesisProgressAnalyticsService', () => {
  let analyticsService: KinesisProgressAnalyticsService;
  ...
  beforeEach(() => {
    ...
    analyticsService = new KinesisProgressAnalyticsService(mockStreamName, 'us-east-1', 5);
  ...
  describe('Kinesis Event Streaming', () => {
    ...
    it('should record progress updates with improvement calculations', async () => {
      const oldMetrics: ProgressMetrics = {
    ...
    it('should handle Kinesis errors gracefully and retain events for retry', async () => {
      ...
      const bufferLength = (analyticsService as any).eventBuffer.length;
  ...
  describe('Progress Calculation Accuracy', () => {
    ...
    it('should calculate streak days correctly', async () => {
      ...
      const consecutiveSessions: ConversationSession[] = [
        {
          sessionId: 'session-1',
          userId: mockUserId,
          agentId: 'friendly-tutor',
    ...
    it('should normalize vocabulary growth correctly', async () => {
      const sessionsWithVocabulary: ConversationSession[] = [];
  ...
  describe('Dashboard Data Aggregation and Visualization', () => {
    ...
    it('should generate contextual recommendations based on performance data', async () => {
      const lowGrammarMetrics: ProgressMetrics = {
    ...
    it('should calculate session frequency accurately', async () => {
      ...
      const frequency = (analyticsService as any).calculateSessionFrequency(recentSessions);
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle network timeouts and retry logic', async () => {
      ...
      await expect((analyticsService as any).flushBuffer()).resolves.not.toThrow();
      
      // Events should still be in buffer
      expect((analyticsService as any).eventBuffer.length).toBeGreaterThan(0);
    });

    it('should validate event data before streaming', async () => {
      // Test with invalid user ID
      await expect(
        analyticsService.recordUserInteraction('', 'test', {})
      ).resolves.not.toThrow();

      // Test with null data
      await expect(
        analyticsService.recordUserInteraction(mockUserId, 'test', null)
      ).resolves.not.toThrow();
    });
    ...
    it('should handle concurrent buffer flushes safely', async () => {
      ...
      const flush1 = (analyticsService as any).flushBuffer();
      const flush2 = (analyticsService as any).flushBuffer();
  ...
  describe('Performance and Scalability', () => {
    ...
    it('should optimize memory usage for large datasets', async () => {
      const largeSessions: ConversationSession[] = [];
      ...
      await expect(
        analyticsService.calculateProgressMetrics(largeSessions)
