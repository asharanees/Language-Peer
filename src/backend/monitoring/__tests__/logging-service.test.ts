import { LoggingService, LogLevel, LogContext } from '../logging-service';
...
describe('LoggingService', () => {
  ...
  beforeEach(() => {
    loggingService = new LoggingService('TestService', 'test');
  ...
  describe('log level filtering', () => {
    test('production environment filters debug logs', () => {
      const prodLogger = new LoggingService('TestService', 'production');
    ...
    test('development environment allows all log levels', () => {
      const devLogger = new LoggingService('TestService', 'development');
  ...
  describe('specialized logging methods', () => {
    test('logConversationEvent includes conversation context', () => {
      ...
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Conversation started"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
      ...
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"component":"conversation"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"agentType":"friendly-tutor"')
      );
    });
    ...
    test('logVoiceEvent includes voice processing context', () => {
      ...
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Voice processing transcription_completed"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"userId":"user-789"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"audioLength":5000')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"component":"voice"')
      );
    });
    ...
    test('logAgentEvent includes agent context', () => {
      ...
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Agent response_generated"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"agentType":"pronunciation-coach"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"sessionId":"session-456"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"component":"agent"')
      );
    });
    ...
    test('logApiRequest includes API context', () => {
      ...
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"message":"API POST /api/conversations"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"method":"POST"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"path":"/api/conversations"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
    ...
    test('logDatabaseOperation logs successful operations', () => {
      ...
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Database query on users succeeded"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"component":"database"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
    ...
    test('logDatabaseOperation logs failed operations as errors', () => {
      ...
      expect(consoleSpy.error).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Database insert on sessions failed"')
      );
      expect(consoleSpy.error).toHaveBeenCalledWith(
        expect.stringContaining('"component":"database"')
      );
  ...
  describe('timer functionality', () => {
    test('createTimer logs operation start and completion', () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Starting test-operation"')
      );

      timer.stop(true, { result: 'success' });

      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"message":"test-operation completed in')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
    ...
    test('timer logs failures as warnings', () => {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        expect.stringContaining('"message":"failing-operation failed in')
      );
      expect(consoleSpy.warn).toHaveBeenCalledWith(
  ...
  describe('logOperation decorator', () => {
    test('decorator automatically logs method execution', async () => {
      class TestService {
        ...
        @loggingService.logOperation('test-method', 'test-component')
        async testMethod(delay: number): Promise<string> {
          await new Promise(resolve => setTimeout(resolve, delay));
          return 'success';
        }

        @loggingService.logOperation('failing-method')
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Starting test-method"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"message":"test-method completed in')
      );

      // Should have logged start and failure for failing method
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Starting failing-method"')
      );
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        expect.stringContaining('"message":"failing-method failed in')
      );
    });
  });
  ...
  describe('utility methods', () => {
    test('logError formats errors consistently', () => {
      ...
      expect(consoleSpy.error).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Error in test-operation: Test error"')
      );
      expect(consoleSpy.error).toHaveBeenCalledWith(
        expect.stringContaining('"userId":"user-123"')
      );
    ...
    test('logUserAction tracks user analytics', () => {
      ...
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"message":"User action: button_click"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"component":"user-analytics"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"buttonId":"start-conversation"')
      );
    });
    ...
    test('logHealthCheck tracks service health', () => {
      ...
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Health check: database is healthy"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"component":"health-check"')
      );
      expect(consoleSpy.info).toHaveBeenCalledWith(
        expect.stringContaining('"responseTime":50')
      );
      ...
      expect(consoleSpy.error).toHaveBeenCalledWith(
        expect.stringContaining('"message":"Health check: api is unhealthy"')
      );
    });
  });
});