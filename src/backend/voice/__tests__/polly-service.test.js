...
describe('PollyService', () => {
    ...
    beforeEach(() => {
        ...
        client_polly_1.PollyClient.mockImplementation(() => mockPollyClient);
        ...
        pollyService = new polly_service_1.PollyService('us-east-1', 'test-audio-bucket');
        // Mock available voices
        mockPollyClient.send.mockImplementation((command) => {
            if (command instanceof client_polly_1.DescribeVoicesCommand) {
    ...
    describe('synthesizeWithPersonality', () => {
        test('synthesizes speech with agent personality matching', async () => {
            ...
            expect(mockPollyClient.send).toHaveBeenCalledWith(expect.any(client_polly_1.SynthesizeSpeechCommand));
            // Verify the command was called with correct voice
            const command = mockPollyClient.send.mock.calls[1][0];
        ...
        test('handles different personality types', async () => {
            ...
            const strictTeacherPersonality = {
                ...mockAgentPersonality,
    ...
    describe('synthesizeSpeech', () => {
        ...
        test('validates input before synthesis', async () => {
            ...
            // Test empty text
            await expect(pollyService.synthesizeSpeech('', config)).rejects.toThrow('Text cannot be empty');
            // Test text too long
            const longText = 'a'.repeat(3001);
            await expect(pollyService.synthesizeSpeech(longText, config)).rejects.toThrow('Text too long');
            // Test invalid voice
            const invalidConfig = { ...config, voiceId: 'InvalidVoice' };
            await expect(pollyService.synthesizeSpeech('Hello', invalidConfig)).rejects.toThrow('Voice InvalidVoice is not available');
        });
    ...
    describe('generatePronunciationGuide', () => {
        test('creates pronunciation guide with SSML', async () => {
            ...
            const result = await pollyService.generatePronunciationGuide('hello', 'həˈloʊ', 'en-US', 2);
            expect(result.audioStream).toEqual(mockAudioBuffer);
            const command = mockPollyClient.send.mock.calls[1][0];
            expect(command.input.TextType).toBe('ssml');
        ...
        test('includes correct number of repetitions', async () => {
            ...
            const command = mockPollyClient.send.mock.calls[1][0];
            const ssmlText = command.input.Text;
    ...
    describe('createConversationalResponse', () => {
        test('creates response with encouragement context', async () => {
            ...
            await pollyService.createConversationalResponse('Fantastic work!', mockAgentPersonality, context);
            const command = mockPollyClient.send.mock.calls[1][0];
            expect(command.input.Text).toContain('<prosody');
            expect(command.input.TextType).toBe('ssml');
        });
        test('creates response with correction context', async () => {
            ...
            await pollyService.createConversationalResponse('Let me help you with that pronunciation.', mockAgentPersonality, context);
            const command = mockPollyClient.send.mock.calls[1][0];
        ...
        test('handles question context with pitch adjustment', async () => {
            ...
            await pollyService.createConversationalResponse('How was your weekend?', mockAgentPersonality, context);
            const command = mockPollyClient.send.mock.calls[1][0];
            expect(command.input.Text).toContain('<prosody');
        });
    });
    describe('storeAudioInS3', () => {
        ...
        test('handles S3 storage errors', async () => {
            ...
            await expect(pollyService.storeAudioInS3(mockAudioBuffer, 'session-123', 'msg-456')).rejects.toThrow('Failed to store audio');
        });
    });
    describe('getVoicesForLanguage', () => {
        test('retrieves voices for specific language', async () => {
            ...
            expect(mockPollyClient.send).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('error handling', () => {
        test('handles Polly service errors', async () => {
            ...
            await expect(pollyService.synthesizeSpeech('Hello', config)).rejects.toThrow('Failed to synthesize speech');
        });
        test('handles missing audio stream', async () => {
            ...
            await expect(pollyService.synthesizeSpeech('Hello', config)).rejects.toThrow('No audio stream received from Polly');
        });
    });
});
