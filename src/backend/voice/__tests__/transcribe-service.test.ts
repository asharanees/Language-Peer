import { TranscribeService, TranscribeConfig } from '../transcribe-service';
...
describe('TranscribeService', () => {
  ...
  let mockClient: jest.Mocked<TranscribeStreamingClient>;
  let mockAudioBuffer: Buffer;
  let mockConfig: TranscribeConfig;
  ...
  beforeEach(() => {
    ...
    (TranscribeStreamingClient as jest.Mock).mockImplementation(() => mockClient);

    // Create service instance
    transcribeService = new TranscribeService('us-east-1');
  ...
  describe('startStreamTranscription', () => {
    test('starts streaming transcription successfully', async () => {
      ...
      expect(mockClient.send).toHaveBeenCalledWith(
        expect.any(StartStreamTranscriptionCommand)
      );
    ...
    test('validates configuration before starting', async () => {
      const invalidConfig = {
        ...mockConfig,
        languageCode: 'invalid-lang' as any
      };

      await expect(
        transcribeService.startStreamTranscription('test-stream', invalidConfig)
    ...
    test('validates sample rate range', async () => {
      ...
      await expect(
        transcribeService.startStreamTranscription('test-stream', invalidConfig)
      ).rejects.toThrow('Sample rate must be between 8000 and 48000 Hz');
    });
    ...
    test('validates number of channels', async () => {
      ...
      await expect(
        transcribeService.startStreamTranscription('test-stream', invalidConfig)
      ).rejects.toThrow('Number of channels must be 1 or 2');
    });
  });
  ...
  describe('transcribeAudioFile', () => {
    ...
    test('handles empty audio buffer', async () => {
      ...
      await expect(
        transcribeService.transcribeAudioFile(emptyBuffer, mockConfig)
      ).rejects.toThrow();
    });
  });
  ...
  describe('assessAudioQuality', () => {
    ...
    test('provides recommendations for low volume', () => {
      ...
      const quietBuffer = Buffer.alloc(AUDIO_CONFIG.SAMPLE_RATE * 2);
      ...
      expect(quality.recommendations).toContain(
        expect.stringContaining('Speak louder')
      );
    });

    test('provides recommendations for high volume', () => {
      // Create very loud audio
      const loudBuffer = Buffer.alloc(AUDIO_CONFIG.SAMPLE_RATE * 2);
  ...
  describe('detectLanguage', () => {
    ...
    test('handles detection errors gracefully', async () => {
      ...
      jest.spyOn(transcribeService, 'transcribeAudioFile').mockRejectedValue(
  ...
  describe('sendAudioData', () => {
    test('sends audio data to active stream', async () => {
      ...
      // Now send audio data
      await expect(
        transcribeService.sendAudioData(streamId, mockAudioBuffer)
      ).resolves.not.toThrow();
    });

    test('throws error for non-existent stream', async () => {
      await expect(
        transcribeService.sendAudioData('non-existent-stream', mockAudioBuffer)
      ).rejects.toThrow('No active stream found');
    });
    ...
    test('validates audio data before sending', async () => {
      ...
      const emptyBuffer = Buffer.alloc(0);
      await expect(
        transcribeService.sendAudioData(streamId, emptyBuffer)
      ...
      await expect(
        transcribeService.sendAudioData(streamId, oddBuffer)
  ...
  describe('stopStreamTranscription', () => {
    ...
    test('throws error for non-existent stream', async () => {
      await expect(
        transcribeService.stopStreamTranscription('non-existent-stream')
      ).rejects.toThrow('No active stream found');
    });
  });
  ...
  describe('getSupportedLanguages', () => {
    test('returns list of supported languages', () => {
      ...
      expect(languages).toEqual(TRANSCRIBE_LANGUAGES);
    });
  });
  ...
  describe('error handling', () => {
    test('handles AWS service errors gracefully', async () => {
      ...
      await expect(
        transcribeService.startStreamTranscription('test-stream', mockConfig)
