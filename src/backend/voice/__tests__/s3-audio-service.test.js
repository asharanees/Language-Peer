...
const s3_audio_service_1 = require("../s3-audio-service");
...
describe('S3AudioService', () => {
    ...
    beforeEach(() => {
        ...
        s3_request_presigner_1.getSignedUrl.mockResolvedValue('https://mock-presigned-url.com');
        // Create service instance
        s3AudioService = new s3_audio_service_1.S3AudioService('test-audio-bucket', 'us-east-1');
    ...
    describe('uploadAudio', () => {
        test('uploads audio file successfully', async () => {
            ...
            const result = await s3AudioService.uploadAudio(mockAudioBuffer, mockMetadata, 'test-user-123', 'test-session-456');
            ...
            expect(mockS3Client.send).toHaveBeenCalledWith(expect.any(client_s3_1.PutObjectCommand));
        });
        test('validates audio data before upload', async () => {
            // Test empty buffer
            const emptyBuffer = Buffer.alloc(0);
            await expect(s3AudioService.uploadAudio(emptyBuffer, mockMetadata)).rejects.toThrow('Audio data cannot be empty');
            // Test oversized buffer
            const oversizedBuffer = Buffer.alloc(60 * 1024 * 1024); // 60MB
            await expect(s3AudioService.uploadAudio(oversizedBuffer, mockMetadata)).rejects.toThrow('Audio file too large');
            // Test unsupported content type
            const invalidMetadata = { ...mockMetadata, contentType: 'video/mp4' };
            await expect(s3AudioService.uploadAudio(mockAudioBuffer, invalidMetadata)).rejects.toThrow('Unsupported audio format');
        });
        test('validates WAV file format', async () => {
            // Create invalid WAV buffer (too small)
            const tinyBuffer = Buffer.alloc(20);
            await expect(s3AudioService.uploadAudio(tinyBuffer, mockMetadata)).rejects.toThrow('Invalid WAV file: too small');
            // Create buffer without proper WAV headers
            const invalidWavBuffer = Buffer.alloc(1024);
            invalidWavBuffer.write('INVALID', 0, 'ascii');
            await expect(s3AudioService.uploadAudio(invalidWavBuffer, mockMetadata)).rejects.toThrow('Invalid WAV file: missing RIFF/WAVE headers');
        });
        test('handles S3 upload errors', async () => {
            mockS3Client.send.mockRejectedValue(new Error('S3 upload failed'));
            await expect(s3AudioService.uploadAudio(mockAudioBuffer, mockMetadata)).rejects.toThrow('Failed to upload audio');
        });
        test('generates correct S3 metadata', async () => {
            mockS3Client.send.mockResolvedValue({});
            await s3AudioService.uploadAudio(mockAudioBuffer, mockMetadata, 'test-user-123', 'test-session-456');
            ...
            expect(putCommand.input.Metadata).toEqual({
                ...
                'duration': '1',
                'sample-rate': constants_1.AUDIO_CONFIG.SAMPLE_RATE.toString(),
                'channels': '1',
    ...
    describe('getAudio', () => {
        test('retrieves audio file successfully', async () => {
            ...
            expect(mockS3Client.send).toHaveBeenCalledWith(expect.any(client_s3_1.HeadObjectCommand));
            expect(mockS3Client.send).toHaveBeenCalledWith(expect.any(client_s3_1.GetObjectCommand));
        });
        test('handles missing audio file', async () => {
            mockS3Client.send.mockRejectedValue(new Error('NoSuchKey'));
            await expect(s3AudioService.getAudio('non-existent-key')).rejects.toThrow('Failed to retrieve audio');
        });
        test('handles missing body in response', async () => {
            ...
            await expect(s3AudioService.getAudio('test-key')).rejects.toThrow('No audio data found');
        });
    });
    describe('deleteAudio', () => {
        test('deletes audio file successfully', async () => {
            ...
            expect(mockS3Client.send).toHaveBeenCalledWith(expect.any(client_s3_1.DeleteObjectCommand));
        });
        test('handles deletion errors', async () => {
            mockS3Client.send.mockRejectedValue(new Error('Access denied'));
            await expect(s3AudioService.deleteAudio('test-key')).rejects.toThrow('Failed to delete audio');
        });
    });
    describe('generateUploadUrl', () => {
        test('generates presigned upload URL', async () => {
            const result = await s3AudioService.generateUploadUrl('test-user-123', 'test-session-456', 'audio/wav', 300);
            expect(result.uploadUrl).toBe('https://mock-presigned-url.com');
            expect(result.audioKey).toMatch(/^audio\/test-user-123\/test-session-456\/\d+_.+\.wav$/);
            expect(result.expiresAt).toBeInstanceOf(Date);
            expect(s3_request_presigner_1.getSignedUrl).toHaveBeenCalledWith(mockS3Client, expect.any(client_s3_1.PutObjectCommand), { expiresIn: 300 });
        });
        test('handles different content types', async () => {
            await s3AudioService.generateUploadUrl('test-user', 'test-session', 'audio/mp3');
            const result = await s3AudioService.generateUploadUrl('test-user', 'test-session', 'audio/mp3');
            expect(result.audioKey).toMatch(/\.mp3$/);
        });
        test('handles presigned URL generation errors', async () => {
            s3_request_presigner_1.getSignedUrl.mockRejectedValue(new Error('URL generation failed'));
            await expect(s3AudioService.generateUploadUrl('user', 'session')).rejects.toThrow('Failed to generate upload URL');
        });
    });
    ...
    describe('analyzeAudioQuality', () => {
        ...
        test('provides recommendations for quiet audio', () => {
            ...
            expect(result.recommendations).toContain(expect.stringContaining('too quiet'));
        });
        test('provides recommendations for loud audio', () => {
            ...
            expect(result.recommendations).toContain(expect.stringContaining('too loud'));
        });
        test('provides recommendations for short audio', () => {
            ...
            expect(result.recommendations).toContain(expect.stringContaining('very short'));
        });
        test('provides recommendations for low sample rate', () => {
            ...
            expect(result.recommendations).toContain(expect.stringContaining('Low sample rate'));
        });
    });
    describe('private helper methods', () => {
        test('getFileExtension returns correct extensions', () => {
            // Access private method through any casting for testing
            const service = s3AudioService;
