...
const fluency_analyzer_1 = require("../fluency-analyzer");
const transcribe_service_1 = require("../../voice/transcribe-service");
...
describe('FluencyAnalyzer', () => {
    ...
    beforeEach(() => {
        ...
        transcribe_service_1.TranscribeService.mockImplementation(() => mockTranscribeService);
        bedrock_client_1.BedrockClient.mockImplementation(() => mockBedrockClient);
        // Create analyzer instance
        fluencyAnalyzer = new fluency_analyzer_1.FluencyAnalyzer('us-east-1');
        fluencyAnalyzer.transcribeService = mockTranscribeService;
        fluencyAnalyzer.bedrockClient = mockBedrockClient;
        // Mock context
    ...
    describe('analyzeFluency', () => {
        test('analyzes fluency with audio and transcription successfully', async () => {
            ...
            const result = await fluencyAnalyzer.analyzeFluency(mockAudioBuffer, testText, mockContext, mockConfig);
            ...
            expect(mockTranscribeService.transcribeAudioFile).toHaveBeenCalledWith(mockAudioBuffer, expect.objectContaining({
                languageCode: 'en-US'
            }));
        ...
        test('analyzes fluency with text only when no audio provided', async () => {
            ...
            mockBedrockClient.invokeModel.mockResolvedValue(mockBedrockResponse);
            const result = await fluencyAnalyzer.analyzeFluency(null, testText, mockContext, mockConfig);
        ...
        test('handles transcription service errors gracefully', async () => {
            ...
            mockTranscribeService.transcribeAudioFile.mockRejectedValue(new Error('Transcription failed'));
            const mockBedrockResponse = {
                content: JSON.stringify({
                    fluencyScore: 0.70,
        ...
        test('applies different strictness levels correctly', async () => {
            ...
            const lenientResult = await fluencyAnalyzer.analyzeFluency(null, testText, mockContext, lenientConfig);
            // Test strict mode
            const strictConfig = { ...mockConfig, strictnessLevel: 'strict' };
            const strictResult = await fluencyAnalyzer.analyzeFluency(null, testText, mockContext, strictConfig);
        ...
        test('adjusts analysis based on user level', async () => {
            ...
            const beginnerResult = await fluencyAnalyzer.analyzeFluency(null, testText, beginnerContext, mockConfig);
            const advancedResult = await fluencyAnalyzer.analyzeFluency(null, testText, advancedContext, mockConfig);
    ...
    describe('pronunciation analysis', () => {
        test('analyzes pronunciation from audio with high confidence', async () => {
            ...
            mockTranscribeService.transcribeAudioFile.mockResolvedValue(mockTranscriptionResult);
            mockTranscribeService.assessAudioQuality.mockReturnValue(mockAudioQuality);
            const result = await fluencyAnalyzer.analyzePronunciation(mockAudioBuffer, testText, mockConfig);
    ...
    describe('rhythm and pace analysis', () => {
        test('analyzes speech rhythm from audio timing', async () => {
            ...
            const mockAudioBuffer = Buffer.alloc(constants_1.AUDIO_CONFIG.SAMPLE_RATE * 3 * 2); // 3 seconds
        ...
        test('detects speech that is too fast', async () => {
            ...
            const mockAudioBuffer = Buffer.alloc(constants_1.AUDIO_CONFIG.SAMPLE_RATE * 0.5 * 2); // 0.5 seconds
            const result = await fluencyAnalyzer.analyzeRhythm(mockAudioBuffer, testText);
        ...
        test('detects speech that is too slow', async () => {
            ...
            const mockAudioBuffer = Buffer.alloc(constants_1.AUDIO_CONFIG.SAMPLE_RATE * 10 * 2); // 10 seconds
            const result = await fluencyAnalyzer.analyzeRhythm(mockAudioBuffer, testText);
        ...
        test('identifies optimal speaking pace', async () => {
            ...
            const mockAudioBuffer = Buffer.alloc(constants_1.AUDIO_CONFIG.SAMPLE_RATE * optimalDuration * 2);
            const result = await fluencyAnalyzer.analyzeRhythm(mockAudioBuffer, testText);
    ...
    describe('fluency scoring algorithms', () => {
        test('calculates composite fluency score correctly', async () => {
            ...
            const result = fluencyAnalyzer.calculateCompositeFluencyScore(mockScores);
        ...
        test('weights pronunciation more heavily for beginners', async () => {
            ...
            const beginnerScore = fluencyAnalyzer.calculateCompositeFluencyScore(mockScores, 'beginner');
            const advancedScore = fluencyAnalyzer.calculateCompositeFluencyScore(mockScores, 'advanced');
        ...
        test('penalizes low transcription confidence appropriately', async () => {
            ...
            const highConfidenceResult = fluencyAnalyzer.calculateCompositeFluencyScore(highConfidenceScores);
            const lowConfidenceResult = fluencyAnalyzer.calculateCompositeFluencyScore(lowConfidenceScores);
    ...
    describe('feedback generation', () => {
        test('generates appropriate pronunciation feedback', async () => {
            ...
            const feedback = fluencyAnalyzer.generatePronunciationFeedback(problemAreas, confidence, 'intermediate');
            expect(feedback.length).toBeGreaterThan(0);
            expect(feedback.some((f) => f.includes('th'))).toBe(true);
            expect(feedback.some((f) => f.includes('r'))).toBe(true);
        });
        test('generates appropriate pace feedback', async () => {
            const wordsPerMinute = 250; // Too fast
            const paceScore = 0.45;
            const feedback = fluencyAnalyzer.generatePaceFeedback(wordsPerMinute, paceScore);
            expect(feedback.length).toBeGreaterThan(0);
            expect(feedback.some((f) => f.includes('slow down'))).toBe(true);
        });
        test('generates encouraging feedback for good performance', async () => {
            ...
            const feedback = fluencyAnalyzer.generateEncouragingFeedback(mockAnalysis);
            expect(feedback.length).toBeGreaterThan(0);
            expect(feedback.some((f) => f.toLowerCase().includes('great'))).toBe(true);
        });
    });
    describe('error handling and edge cases', () => {
        test('handles empty text input', async () => {
            const result = await fluencyAnalyzer.analyzeFluency(null, '', mockContext, mockConfig);
        ...
        test('handles very short audio input', async () => {
            ...
            const testText = 'Hi.';
            const result = await fluencyAnalyzer.analyzeFluency(shortAudio, testText, mockContext, mockConfig);
        ...
        test('handles mismatched audio and text length', async () => {
            const shortText = 'Hi.';
            const longAudio = Buffer.alloc(constants_1.AUDIO_CONFIG.SAMPLE_RATE * 10 * 2); // 10 seconds
            ...
            const result = await fluencyAnalyzer.analyzeFluency(longAudio, shortText, mockContext, mockConfig);
