import { GrammarAnalyzer, GrammarAnalysisConfig } from '../grammar-analyzer';
...
import { ConversationContext, UserProfile, LanguageAnalysis } from '@/shared/types';
...
describe('GrammarAnalyzer', () => {
  ...
  beforeEach(() => {
    ...
    (ComprehendClient as jest.Mock).mockImplementation(() => mockComprehendClient);
    (BedrockClient as jest.Mock).mockImplementation(() => mockBedrockClient);

    // Create analyzer instance
    grammarAnalyzer = new GrammarAnalyzer('us-east-1');
    (grammarAnalyzer as any).comprehendClient = mockComprehendClient;
    (grammarAnalyzer as any).bedrockClient = mockBedrockClient;
  ...
  describe('analyzeGrammar', () => {
    ...
    test('focuses on specified error types', async () => {
      ...
      const grammarOnlyConfig = { ...mockConfig, focusAreas: ['grammar'] as const };
  ...
  describe('rule-based grammar checking', () => {
    test('detects subject-verb agreement errors', async () => {
      ...
      const agreementErrors = result.errors.filter(e => 
        e.description.includes('Subject and verb must agree')
      );
    ...
    test('detects article usage errors', async () => {
      ...
      const articleErrors = result.errors.filter(e => 
        e.description.includes('article usage')
      );
    ...
    test('detects double negatives', async () => {
      ...
      const doubleNegErrors = result.errors.filter(e => 
        e.description.includes('double negative')
      );
    ...
    test('detects run-on sentences', async () => {
      ...
      const runOnErrors = result.errors.filter(e => 
        e.description.includes('too long')
      );
  ...
  describe('error prioritization and suggestions', () => {
    ...
    test('limits number of errors based on strictness', async () => {
      ...
      const lenientConfig = { ...mockConfig, strictnessLevel: 'lenient' as const, enableContextualAnalysis: false };
      const strictConfig = { ...mockConfig, strictnessLevel: 'strict' as const, enableContextualAnalysis: false };
