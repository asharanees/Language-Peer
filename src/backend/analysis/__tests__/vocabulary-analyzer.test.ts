import { VocabularyAnalyzer, VocabularyAnalysisConfig } from '../vocabulary-analyzer';
...
import { ConversationContext, UserProfile } from '@/shared/types';
...
describe('VocabularyAnalyzer', () => {
  ...
  beforeEach(() => {
    ...
    vocabularyAnalyzer = new VocabularyAnalyzer('us-east-1');
    (vocabularyAnalyzer as any).comprehendClient = mockComprehendClient;
    (vocabularyAnalyzer as any).bedrockClient = mockBedrockClient;
  ...
  describe('analyzeVocabulary', () => {
    ...
    test('handles different complexity levels correctly', async () => {
      ...
      const beginnerResult = await vocabularyAnalyzer.analyzeVocabulary(
        beginnerText,
        mockContext,
        { ...mockConfig, targetLevel: 'beginner' }
      );

      const advancedResult = await vocabularyAnalyzer.analyzeVocabulary(
        advancedText,
        mockContext,
        { ...mockConfig, targetLevel: 'advanced' }
      );
    ...
    test('generates appropriate suggestions for different target levels', async () => {
      ...
      const advancedConfig = { ...mockConfig, targetLevel: 'advanced' as const };
  ...
  describe('vocabulary complexity analysis', () => {
    test('correctly identifies word complexity levels', async () => {
      const analyzer = vocabularyAnalyzer as any;
    ...
    test('assesses appropriateness for context', async () => {
      ...
      const travelScore = analyzer.calculateAppropriatenessScore(
        travelWords, 
        'Travel and Culture', 
        ['casual']
      );
      
      const businessScore = analyzer.calculateAppropriatenessScore(
        businessWords, 
        'Business Meeting', 
        ['business']
      );
  ...
  describe('suggestion generation', () => {
    ...
    test('suggests simpler alternatives for complex words when appropriate', async () => {
      ...
      const beginnerConfig = { ...mockConfig, targetLevel: 'beginner' as const };
