...
exports.AutonomousConversationOrchestrator = void 0;
class AutonomousConversationOrchestrator {
    constructor(bedrockClient) {
        this.SILENCE_THRESHOLD = 10000; // 10 seconds in milliseconds
        this.LOW_ENGAGEMENT_THRESHOLD = 30; // engagement score below 30
        this.HIGH_FRUSTRATION_THRESHOLD = 0.7; // frustration level above 70%
        this.bedrockClient = bedrockClient;
    }
    /**
     * Autonomously select conversation topics based on user profile and history
     */
    async selectTopicAutonomously(userProfile, sessionHistory, currentContext) {
        try {
            ...
            const response = await this.bedrockClient.invokeModel('You are an AI language learning assistant. Select appropriate conversation topics.', prompt, context);
     ...
     */
    async selectTopicWithAdvancedAlgorithms(userProfile, sessionHistory, currentEngagement, timeOfDay) {
        ...
        const engagementAdjustedTopics = this.adjustTopicsForEngagement(userProfile.conversationTopics, currentEngagement);
        // Combine all factors for optimal topic selection
        const weightedTopics = this.combineTopicFactors(topicPreferences, timeBasedTopics, engagementAdjustedTopics, userProfile);
     ...
     */
    async adjustDifficultyAutonomously(userProfile, recentMessages, performanceMetrics) {
        try {
            ...
            const response = await this.bedrockClient.invokeModel('You are an AI language learning assistant. Analyze performance and recommend difficulty adjustments.', prompt, context);
     ...
     */
    async adjustDifficultyWithRealTimeAnalysis(userProfile, recentMessages, performanceMetrics, sessionDuration, errorPatterns) {
        ...
        return await this.generateAdaptiveDifficultyRecommendation(userProfile, performanceTrend, strugglingAreas, fatigueLevel);
    }
    /**
     * Detect user engagement and generate appropriate responses
     */
    async detectEngagementAndRespond(conversationHistory, lastUserActivity, userProfile) {
     ...
     */
    async detectEngagementWithBehavioralAnalysis(conversationHistory, lastUserActivity, userProfile, sessionMetrics) {
        ...
        const engagementLevel = this.calculateComprehensiveEngagement(conversationPatterns, emotionalState, timingPatterns, sessionMetrics);
        // Generate targeted interventions
        const interventionRecommendations = await this.generateTargetedInterventions(engagementLevel, conversationPatterns, emotionalState, userProfile);
     ...
     */
    async generateContinuationPrompt(conversationHistory, currentTopic, userProfile) {
        try {
            ...
            const response = await this.bedrockClient.invokeModel('You are an AI language learning assistant. Generate natural conversation continuations.', prompt, context);
            return {
                type: 'continuation',
     ...
     */
    analyzePerformanceTrends(sessionHistory, timeframe) {
        ...
        const performanceScores = recentSessions.map(session => this.calculateSessionPerformanceScore(session));
    ...
    analyzeTopicPreferences(sessionHistory) {
    ...
    adjustTopicsForEngagement(userTopics, currentEngagement) {
        if (currentEngagement < 30) {
            // Low engagement - suggest easier, more interesting topics
            return userTopics.filter(topic => ['Hobbies', 'Entertainment', 'Food', 'Travel'].some(easy => topic.toLowerCase().includes(easy.toLowerCase())));
        }
    ...
    combineTopicFactors(preferences, timeBasedTopics, engagementTopics, userProfile) {
    ...
    async selectOptimalTopic(weightedTopics, userProfile, sessionHistory) {
    ...
    analyzePerformanceTrend(recentMessages, performanceMetrics) {
    ...
    identifyStrugglingAreas(errorPatterns, performanceMetrics) {
    ...
    calculateFatigueLevel(sessionDuration, recentMessages) {
    ...
    async generateAdaptiveDifficultyRecommendation(userProfile, performanceTrend, strugglingAreas, fatigueLevel) {
        let recommendedDifficulty = userProfile.currentLevel;
        let adjustmentReason = 'Maintaining current difficulty';
        let adjustmentStrength = 'minor';
    ...
    analyzeResponseTimingPatterns(conversationHistory) {
        const responseTimes = [];
    ...
    calculateComprehensiveEngagement(conversationPatterns, emotionalState, timingPatterns, sessionMetrics) {
        let score = 50; // baseline
        // Message length factor
        if (conversationPatterns.averageMessageLength > 8)
            score += 15;
        else if (conversationPatterns.averageMessageLength < 3)
            score -= 15;
        ...
        if (timingPatterns.averageResponseTime < 5000)
            score += 10;
        else if (timingPatterns.averageResponseTime > 15000)
            score -= 15;
        if (timingPatterns.trend === 'slowing')
            score -= 10;
        else if (timingPatterns.trend === 'accelerating')
            score += 5;
    ...
    async generateTargetedInterventions(engagementLevel, conversationPatterns, emotionalState, userProfile) {
        const interventions = [];
    ...
    identifyBehavioralPatterns(conversationHistory, sessionMetrics) {
        const patterns = [];
        const userMessages = conversationHistory.filter(msg => msg.sender === 'user');
        if (userMessages.length > 0) {
            const avgLength = userMessages.reduce((sum, msg) => sum + msg.content.split(' ').length, 0) / userMessages.length;
            if (avgLength < 3)
                patterns.push('minimal_responses');
            if (avgLength > 15)
                patterns.push('verbose_responses');
        ...
        if (sessionMetrics) {
            if (sessionMetrics.grammarAccuracy < 50)
                patterns.push('grammar_struggles');
            if (sessionMetrics.fluencyScore < 50)
                patterns.push('fluency_challenges');
        }
        return patterns;
    }
    identifyRiskFactors(engagementLevel, emotionalState, timingPatterns) {
        const riskFactors = [];
        if (engagementLevel < 25)
        ...
        if (timingPatterns.averageResponseTime > 20000)
            riskFactors.push('slow_responses');
        if (timingPatterns.trend === 'slowing')
            riskFactors.push('declining_responsiveness');
        return riskFactors;
    }
    estimateTopicDuration(topic, level) {
        const baseDurations = {
            'beginner': 10,
            'elementary': 12,
            'intermediate': 15,
    ...
    buildTopicSelectionPrompt(userProfile, sessionHistory, currentContext) {
...
Respond in JSON format:
{
  ...
  "estimatedDuration": minutes
    ...
    buildDifficultyAdjustmentPrompt(userProfile, currentPerformance) {
        return `
...
Respond in JSON format:
{
  ...
  "adjustmentStrength": "minor|moderate|major"
    ...
    buildContinuationPrompt(conversationHistory, currentTopic, userProfile) {
...
Respond with just the continuation message, no JSON format needed.
    ...
    parseTopicRecommendation(response, userProfile) {
        try {
    ...
    parseDifficultyAdjustment(response, currentLevel) {
        try {
            const parsed = JSON.parse(response);
            return {
    ...
    calculateEngagementMetrics(conversationHistory, lastUserActivity) {
        const userMessages = conversationHistory.filter(msg => msg.sender === 'user');
        if (userMessages.length === 0) {
            return {
                responseTime: 0,
        ...
        const avgMessageLength = userMessages.reduce((sum, msg) => sum + msg.content.split(' ').length, 0) / userMessages.length;
        const avgConfidence = userMessages.reduce((sum, msg) => sum + (msg.transcriptionConfidence || 0.5), 0) / userMessages.length;
    ...
    async generateSilencePrompt(conversationHistory, userProfile) {
        const prompts = [
    ...
    async generateEngagementPrompt(conversationHistory, metrics, userProfile) {
        const prompts = [
    ...
    async generateFrustrationSupportPrompt(conversationHistory, userProfile) {
        const prompts = [
    ...
    getFallbackTopicRecommendation(userProfile) {
    ...
    getFallbackContinuationPrompt(currentTopic) {
        return {
            type: 'continuation',
    ...
    analyzeCurrentPerformance(messages, metrics) {
        return {
    ...
    detectFrustrationLevel(userMessages) {
        ...
        const hasfrustrationKeywords = userMessages.some(msg => frustrationKeywords.some(keyword => msg.content.toLowerCase().includes(keyword)));
        return hasfrustrationKeywords ? 'medium' : 'low';
    }
    calculateEngagementScore(metrics) {
        let score = 50; // baseline
        // Adjust based on response time (faster = more engaged)
        if (metrics.responseTime && metrics.responseTime < 5)
            score += 20;
        else if (metrics.responseTime && metrics.responseTime > 15)
            score -= 20;
        // Adjust based on message length (longer = more engaged)
        if (metrics.messageLength && metrics.messageLength > 10)
            score += 15;
        else if (metrics.messageLength && metrics.messageLength < 3)
            score -= 15;
        // Adjust based on confidence
        if (metrics.confidenceScore && metrics.confidenceScore > 0.8)
            score += 15;
        else if (metrics.confidenceScore && metrics.confidenceScore < 0.5)
            score -= 15;
        return Math.max(0, Math.min(100, score));
    }
    ...
    generatePerformanceRecommendations(trend, sessions) {
        switch (trend) {
            case 'improving':
                return [
...
exports.AutonomousConversationOrchestrator = AutonomousConversationOrchestrator;
