...
exports.BedrockRecommendationEngine = void 0;
const bedrock_client_1 = require("./bedrock-client");
const constants_1 = require("@/shared/constants");
class BedrockRecommendationEngine {
    constructor(region) {
     ...
     */
    async recommendTopics(userProfile, sessionHistory) {
        try {
            ...
            const bedrockRecommendations = await this.getBedrockTopicRecommendations(userProfile, topicPerformance, recentTopics);
            // Combine with rule-based recommendations
            const ruleBasedRecommendations = this.getRuleBasedTopicRecommendations(userProfile, recentTopics);
     ...
     */
    async recommendAgents(learningGoals, currentProgress) {
        try {
     ...
     */
    async recommendDifficulty(userProfile, recentSessions) {
        try {
            ...
            const bedrockRecommendation = await this.getBedrockDifficultyRecommendation(userProfile, adjustmentFactors, recentSessions);
     ...
     */
    async generateSessionRecommendations(userProfile, sessionHistory) {
        try {
     ...
     */
    async adaptToFeedback(userId, feedback) {
        try {
     ...
     */
    analyzeTopicPerformance(sessionHistory) {
        const topicPerformance = new Map();
        sessionHistory.forEach(session => {
            const topic = session.topic;
            const performance = ((session.performanceMetrics?.grammarAccuracy || 0) +
                (session.performanceMetrics?.fluencyScore || 0)) / 2;
    ...
    getRecentTopics(sessionHistory, count) {
        return sessionHistory
            .slice(0, count)
            .map(session => session.topic);
    }
    async getBedrockTopicRecommendations(userProfile, topicPerformance, recentTopics) {
        try {
            const prompt = this.buildTopicRecommendationPrompt(userProfile, topicPerformance, recentTopics);
            const response = await this.bedrockClient.invokeModel('You are a language learning expert providing topic recommendations.', prompt, {
    ...
    getRuleBasedTopicRecommendations(userProfile, recentTopics) {
        const recommendations = [];
        const availableTopics = [...constants_1.CONVERSATION_TOPICS].filter((topic) => !recentTopics.includes(topic));
    ...
    calculateSessionFrequency(recentSessions) {
        if (recentSessions.length === 0)
            return 0;
        const now = new Date();
        const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
        const recentSessionCount = recentSessions.filter(session => new Date(session.startTime) > weekAgo).length;
        return Math.min(1, recentSessionCount / 7); // Normalize to 0-1
    }
    calculateErrorRate(recentSessions) {
        if (recentSessions.length === 0)
            return 0.5;
        const totalErrors = recentSessions.reduce((sum, session) => sum + (session.performanceMetrics?.errorsCount || 0), 0);
        const totalWords = recentSessions.reduce((sum, session) => sum + (session.performanceMetrics?.wordsSpoken || 0), 0);
        return totalWords > 0 ? totalErrors / totalWords : 0.5;
    }
    async getBedrockDifficultyRecommendation(userProfile, adjustmentFactors, recentSessions) {
        try {
            const prompt = this.buildDifficultyRecommendationPrompt(userProfile, adjustmentFactors, recentSessions);
            const response = await this.bedrockClient.invokeModel('You are a language learning expert providing difficulty recommendations.', prompt, {
                sessionId: 'recommendation-session',
                userId: userProfile.userId,
                conversationHistory: [],
                userProfile: {
    ...
    recommendSessionLength(userProfile, sessionHistory) {
    ...
    identifyFocusAreas(progressMetrics, learningGoals) {
        const focusAreas = [];
    ...
    async generateMotivationalMessage(userProfile, sessionHistory) {
        try {
    ...
    getDifficultyForTopic(topic, userLevel) {
    ...
    rankTopicRecommendations(recommendations, userProfile) {
    ...
    getFallbackTopicRecommendations(userProfile) {
    ...
    getFallbackAgentRecommendations() {
        return [{
    ...
    getDefaultDifficultyRecommendation(level) {
        const difficultyMap = {
            'beginner': 'easy',
            'elementary': 'easy',
            'intermediate': 'medium',
        ...
        return {
            recommendedDifficulty: difficultyMap[level] || 'medium',
            confidence: 0.7,
    ...
    buildTopicRecommendationPrompt(userProfile, topicPerformance, recentTopics) {
        return `
...
Available Topics: ${constants_1.CONVERSATION_TOPICS.join(', ')}
...
`;
    ...
    buildDifficultyRecommendationPrompt(userProfile, adjustmentFactors, recentSessions) {
        return `
...
`;
    ...
    parseDifficultyRecommendation(content, adjustmentFactors) {
        try {
            const jsonMatch = content.match(/\{[\s\S]*\}/);
            if (jsonMatch) {
    ...
    async updateRecommendationWeights(userId, feedbackAnalysis) {
...
exports.BedrockRecommendationEngine = BedrockRecommendationEngine;
