import { DynamoDBConversationOrchestrator } from '../conversation-orchestrator';
import { DynamoDBDocumentClient } from '@aws-sdk/lib-dynamodb';
import { AgentFactory } from '@/agents/coordination/agent-factory';
import { FriendlyTutorAgent } from '@/agents/personalities/friendly-tutor';
import { AGENT_PERSONALITIES } from '@/shared/constants';
import { UserProfile, CreateSessionRequest, SendMessageRequest } from '@/shared/types';
...
describe('DynamoDBConversationOrchestrator', () => {
  ...
  beforeEach(() => {
    ...
    orchestrator = new DynamoDBConversationOrchestrator(
      'test-users-table',
      'test-sessions-table',
      'us-east-1'
    );
    ...
    (orchestrator as any).dynamoClient = mockDynamoClient;
  ...
  describe('createSession', () => {
    test('creates a new conversation session successfully', async () => {
      ...
      expect(result.agentPersonality.id).toBe(AGENT_PERSONALITIES.FRIENDLY_TUTOR);
  ...
  describe('sendMessage', () => {
    test('processes user message and generates agent response', async () => {
      ...
      (orchestrator as any).sessionContexts.set(sessionId, {
        sessionId,
        userId: 'test-user-123',
        conversationHistory: [],
      ...
      (orchestrator as any).activeAgents.set(sessionId, mockAgent);
  ...
  describe('endSession', () => {
    test('ends session and updates metrics', async () => {
      ...
      expect((orchestrator as any).activeAgents.has(sessionId)).toBe(false);
      expect((orchestrator as any).sessionContexts.has(sessionId)).toBe(false);
    });
  });
