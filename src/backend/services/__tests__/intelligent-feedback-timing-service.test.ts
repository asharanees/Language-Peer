import { IntelligentFeedbackTimingService } from '../intelligent-feedback-timing-service';
import { BedrockService } from '../bedrock-client';
import { EngagementDetectionService, EngagementAnalysis } from '../engagement-detection-service';
import { 
  ConversationMessage, 
  UserProfile, 
  LanguageAnalysis, 
...
describe('IntelligentFeedbackTimingService', () => {
  let service: IntelligentFeedbackTimingService;
  let mockBedrockClient: jest.Mocked<BedrockService>;
  let mockEngagementService: jest.Mocked<EngagementDetectionService>;
  ...
  const mockLanguageAnalysis: LanguageAnalysis = {
    grammarScore: 75,
    fluencyScore: 80,
    vocabularyLevel: 65,
    detectedErrors: [
  ...
  beforeEach(() => {
    ...
    service = new IntelligentFeedbackTimingService(mockBedrockClient, mockEngagementService);
  });

  describe('determineFeedbackTiming', () => {
    it('should determine appropriate feedback timing for grammar errors', async () => {
      const mockEngagementAnalysis: EngagementAnalysis = {
      ...
      const result = await service.determineFeedbackTiming(
      ...
      expect(result.feedbackType).toBe(FEEDBACK_TYPES.CORRECTION);
    ...
    it('should provide encouragement when user needs motivation', async () => {
      ...
      const noErrorAnalysis: LanguageAnalysis = {
        ...mockLanguageAnalysis,
        detectedErrors: []
      };
      ...
      if (result.shouldProvideFeedback) {
        expect(result.feedbackType).toBe(FEEDBACK_TYPES.ENCOURAGEMENT);
      }
    });
  });
  ...
  describe('prioritizeErrors', () => {
    ...
    it('should prioritize critical errors higher', () => {
      const multipleErrorAnalysis: LanguageAnalysis = {
        ...mockLanguageAnalysis,
        detectedErrors: [
          {
            type: 'pronunciation',
  ...
  describe('assessMotivationalContext', () => {
    it('should assess user motivational context correctly', () => {
      ...
      const result = service.assessMotivationalContext(
        mockConversationHistory,
        mockSessionMetrics,
        mockEngagementAnalysis
      );
    ...
    it('should detect high frustration from poor performance', () => {
      const poorSessionMetrics: SessionMetrics = {
        ...mockSessionMetrics,
        grammarAccuracy: 30,
        fluencyScore: 25,
        errorsCount: 15,
  ...
  describe('generateMotivationalSupport', () => {
    it('should generate appropriate motivational messages', async () => {
      const mockMotivationalContext = {
        ...
        motivationType: 'effort' as const
      };
      ...
      const result = await service.generateMotivationalSupport(
        mockMotivationalContext,
        mockUserProfile,
        {}
      );
    ...
    it('should provide fallback message when AI service fails', async () => {
      const mockMotivationalContext = {
        ...
        motivationType: 'progress' as const
      };
  ...
  describe('createCorrectionStrategy', () => {
    it('should create appropriate correction strategies', async () => {
      ...
      const result = await service.createCorrectionStrategy(
        mockErrorPriorities,
        mockUserProfile,
        {}
      );
  ...
  describe('edge cases and error handling', () => {
    it('should handle missing language analysis gracefully', async () => {
      const emptyAnalysis: LanguageAnalysis = {
        grammarScore: 0,
        fluencyScore: 0,
        vocabularyLevel: 0,
        detectedErrors: [],
