import { AutonomousConversationOrchestrator } from '../autonomous-conversation-orchestrator';
import { BedrockService } from '../bedrock-client';
import { UserProfile, ConversationSession, ConversationMessage } from '../../../shared/types';

// Mock the BedrockService
jest.mock('../bedrock-client');

describe('AutonomousConversationOrchestrator', () => {
  let orchestrator: AutonomousConversationOrchestrator;
  let mockBedrockClient: jest.Mocked<BedrockService>;
  ...
  const mockSessionHistory: ConversationSession[] = [
    {
      sessionId: 'session-1',
      userId: 'user-123',
      agentId: 'agent-1',
  ...
  beforeEach(() => {
    mockBedrockClient = new BedrockService() as jest.Mocked<BedrockService>;
    orchestrator = new AutonomousConversationOrchestrator(mockBedrockClient);
  });
  ...
  describe('selectTopicAutonomously', () => {
    it('should select appropriate topic based on user profile', async () => {
      ...
      const result = await orchestrator.selectTopicAutonomously(
        mockUserProfile,
        mockSessionHistory
      );
      ...
      expect(mockBedrockClient.invokeModel).toHaveBeenCalledWith(
        expect.stringContaining('AI language learning assistant'),
        expect.any(String),
    ...
    it('should consider recent topics to avoid repetition', async () => {
      ...
      expect(mockBedrockClient.invokeModel).toHaveBeenCalledWith(
        expect.any(String),
        expect.stringContaining('Travel Planning'),
        expect.any(Object)
      );
    });
  });
  ...
  describe('adjustDifficultyAutonomously', () => {
    ...
    it('should recommend difficulty increase for high performance', async () => {
      ...
      const result = await orchestrator.adjustDifficultyAutonomously(
        mockUserProfile,
        mockMessages,
        performanceMetrics
      );
  ...
  describe('detectEngagementAndRespond', () => {
    ...
    it('should detect silence and generate appropriate prompt', async () => {
      ...
      const result = await orchestrator.detectEngagementAndRespond(
        mockMessages,
        lastActivity,
        mockUserProfile
      );
    ...
    it('should detect frustration and provide support', async () => {
      const frustratedMessages: ConversationMessage[] = [
        {
          id: 'msg-1',
          type: 'user',
  ...
  describe('generateContinuationPrompt', () => {
    ...
    it('should generate natural continuation prompt', async () => {
      ...
      const result = await orchestrator.generateContinuationPrompt(
        mockMessages,
        'Travel Experiences',
        mockUserProfile
      );
  ...
  describe('analyzePerformanceTrends', () => {
    it('should identify improving trend', () => {
      const improvingSessions: ConversationSession[] = [
        {
      ...
      expect(result.recommendations).toContain(
        expect.stringMatching(/increasing.*difficulty/i)
      );
    });

    it('should identify declining trend', () => {
      const decliningSessions: ConversationSession[] = [
        {
          ...mockSessionHistory[0],
          sessionId: 'session-1',
          performanceMetrics: { ...mockSessionHistory[0].performanceMetrics, overallScore: 85 }
      ...
      expect(result.recommendations).toContain(
        expect.stringMatching(/reviewing.*feedback/i)
      );
    });
  ...
  describe('error handling and resilience', () => {
    it('should handle Bedrock API timeouts gracefully', async () => {
      mockBedrockClient.generateResponse.mockImplementation(() => 
    ...
    it('should provide meaningful fallbacks for all operations', async () => {
      ...
      const topicResult = await orchestrator.selectTopicAutonomously(
        mockUserProfile,
        mockSessionHistory
      );

      const difficultyResult = await orchestrator.adjustDifficultyAutonomously(
        mockUserProfile,
        [],
        {}
      );

      const continuationResult = await orchestrator.generateContinuationPrompt(
        [],
        'Test Topic',
        mockUserProfile
      );
  ...
  describe('Topic Selection Algorithm Tests', () => {
    it('should select beginner topics for beginner users', async () => {
      ...
      expect(mockBedrockClient.generateResponse).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('should provide variety when user has limited topic preferences', async () => {
      ...
      const fallbackResult = await orchestrator.selectTopicAutonomously(
        limitedProfile,
        []
      );
  ...
  describe('Difficulty Adjustment Algorithm Tests', () => {
    ...
    it('should consider user level constraints in difficulty adjustment', async () => {
      ...
      const result = await orchestrator.adjustDifficultyAutonomously(
        beginnerProfile,
        [],
        { grammarScore: 95, fluencyScore: 95, confidence: 0.95, errorRate: 0.02 }
      );
  ...
  describe('Engagement Detection and Response Tests', () => {
    ...
    it('should detect low engagement from response patterns', async () => {
      const lowEngagementMessages: ConversationMessage[] = [
        {
          id: 'msg-1',
          type: 'user',
          content: 'Yes',
    ...
    it('should not intervene when engagement is healthy', async () => {
      const healthyMessages: ConversationMessage[] = [
        {
          id: 'msg-1',
          type: 'user',
  ...
  describe('Performance Trend Analysis Tests', () => {
    ...
    it('should calculate trend confidence based on data consistency', () => {
      const consistentSessions: ConversationSession[] = [
        {
          ...mockSessionHistory[0],
          sessionId: 'session-1',
          performanceMetrics: { ...mockSessionHistory[0].performanceMetrics, overallScore: 70 }
      ...
      const inconsistentSessions: ConversationSession[] = [
        {
          ...mockSessionHistory[0],
          sessionId: 'session-1',
    ...
    it('should filter sessions by timeframe correctly', () => {
      const mixedTimeSessions: ConversationSession[] = [
        {
          ...mockSessionHistory[0],
          sessionId: 'old-session',
  ...
  describe('Autonomous Decision Making Integration Tests', () => {
    it('should make coordinated decisions across topic, difficulty, and engagement', async () => {
      ...
      const strugglingProfile: UserProfile = {
        ...mockUserProfile,
        currentLevel: 'intermediate'
      };

      const strugglingMessages: ConversationMessage[] = [
        {
          id: 'msg-1',
          type: 'user',
      ...
      const engagementResult = await orchestrator.detectEngagementAndRespond(
        strugglingMessages,
        new Date(),
        strugglingProfile
      );
