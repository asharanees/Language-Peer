import { BedrockService, ConversationContext } from '../bedrock-client';
...
describe('BedrockService', () => {
  ...
  let mockRuntimeClient: jest.Mocked<BedrockRuntimeClient>;
  let mockClient: jest.Mocked<BedrockClient>;

  beforeEach(() => {
    mockRuntimeClient = new BedrockRuntimeClient({}) as jest.Mocked<BedrockRuntimeClient>;
    mockClient = new BedrockClient({}) as jest.Mocked<BedrockClient>;
    
    (BedrockRuntimeClient as jest.Mock).mockReturnValue(mockRuntimeClient);
    (BedrockClient as jest.Mock).mockReturnValue(mockClient);
    
    bedrockService = new BedrockService('us-east-1');
  });
  ...
  describe('Model Selection', () => {
    test('selects Claude 3.5 Sonnet for advanced users', () => {
      ...
      expect(selectedModel).toBe(BEDROCK_MODELS.CLAUDE_3_5_SONNET);
    });
    ...
    test('selects Nova Pro for pronunciation improvement', () => {
      ...
      const selectedModel = bedrockService.selectModel(context);
      expect(selectedModel).toBe(BEDROCK_MODELS.NOVA_PRO);
    });
    ...
    test('selects Llama 3.1 for general conversation', () => {
      ...
      const selectedModel = bedrockService.selectModel(context);
      expect(selectedModel).toBe(BEDROCK_MODELS.LLAMA_3_1_405B);
    });
  ...
  describe('Model Configuration', () => {
    test('returns correct configuration for Claude 3.5 Sonnet', () => {
      ...
      const config = (bedrockService as any).getModelConfig(BEDROCK_MODELS.CLAUDE_3_5_SONNET, context);
    ...
    test('returns correct configuration for Llama 3.1', () => {
      ...
      const config = (bedrockService as any).getModelConfig(BEDROCK_MODELS.LLAMA_3_1_405B, context);
      
      expect(config).toEqual({
        modelId: BEDROCK_MODELS.LLAMA_3_1_405B,
        maxTokens: 1200,
        temperature: 0.6,
        topP: 0.9
      });
    ...
    test('returns correct configuration for Nova Pro', () => {
      ...
      const config = (bedrockService as any).getModelConfig(BEDROCK_MODELS.NOVA_PRO, context);
      
      expect(config).toEqual({
        modelId: BEDROCK_MODELS.NOVA_PRO,
        maxTokens: 800,
        temperature: 0.5,
        topP: 0.9
      });
  ...
  describe('Prompt Formatting', () => {
    test('formats Claude prompt correctly', () => {
      ...
      const prompt = (bedrockService as any).formatPrompt(
  ...
  describe('Request Body Building', () => {
    test('builds Claude request body correctly', () => {
      ...
      const requestBody = (bedrockService as any).buildRequestBody(
        BEDROCK_MODELS.CLAUDE_3_5_SONNET,
        'Test prompt',
        config
      );
  ...
  describe('Response Parsing', () => {
    test('parses Claude response correctly', () => {
      ...
      const parsed = (bedrockService as any).parseResponse(BEDROCK_MODELS.CLAUDE_3_5_SONNET, responseBody);
    ...
    test('parses Llama response correctly', () => {
      ...
      const parsed = (bedrockService as any).parseResponse(BEDROCK_MODELS.LLAMA_3_1_405B, responseBody);
    ...
    test('parses Nova Pro response correctly', () => {
      ...
      const parsed = (bedrockService as any).parseResponse(BEDROCK_MODELS.NOVA_PRO, responseBody);
  ...
  describe('Error Handling', () => {
    test('handles model invocation errors gracefully', async () => {
      ...
      await expect(
        bedrockService.invokeModel('Test prompt', 'Test message', context)
    ...
    test('handles list models errors gracefully', async () => {
      ...
      ).rejects.toThrow('Failed to retrieve available models');
    });
  });
});