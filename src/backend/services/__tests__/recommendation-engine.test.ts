import { BedrockRecommendationEngine } from '../recommendation-engine';
...
import { AGENT_PERSONALITIES, CONVERSATION_TOPICS } from '@/shared/constants';

// Mock the BedrockClient
jest.mock('../bedrock-client');

describe('BedrockRecommendationEngine', () => {
  let recommendationEngine: BedrockRecommendationEngine;
  ...
  const mockUserProfile: UserProfile = {
    ...
    currentLevel: 'intermediate' as LanguageLevel,
    learningGoals: ['conversation-fluency', 'grammar-accuracy'] as LearningGoal[],
    preferredAgents: [AGENT_PERSONALITIES.FRIENDLY_TUTOR],
  ...
  beforeEach(() => {
    ...
    (BedrockClient as jest.MockedClass<typeof BedrockClient>).mockImplementation(() => mockBedrockClient);
    
    recommendationEngine = new BedrockRecommendationEngine('us-east-1');
  });
  ...
  describe('recommendAgents', () => {
    ...
    it('should match agents to specific learning goals', async () => {
      const grammarGoals: LearningGoal[] = ['grammar-accuracy'];
      ...
      const strictTeacher = recommendations.find(r => r.agentId === AGENT_PERSONALITIES.STRICT_TEACHER);
    ...
    it('should return fallback recommendations on error', async () => {
      ...
      const recommendations = await recommendationEngine.recommendAgents([], {} as ProgressMetrics);

      expect(recommendations).toHaveLength(1);
      expect(recommendations[0].agentId).toBe(AGENT_PERSONALITIES.FRIENDLY_TUTOR);
    });
  });
  ...
  describe('recommendDifficulty', () => {
    it('should recommend difficulty based on recent performance', async () => {
      ...
      const recommendation = await recommendationEngine.recommendDifficulty(
        mockUserProfile,
        mockSessionHistory
      );
  ...
  describe('error handling', () => {
    ...
    it('should validate input parameters', async () => {
      ...
      const recommendations = await recommendationEngine.recommendTopics(
        { ...mockUserProfile, learningGoals: [] },
        []
      );
