...
import { generateMessageId, getRandomTopic } from '@/shared/utils';

export class ConversationPartnerAgent extends StrandsAgent {
  constructor(region?: string) {
    const personality: AgentPersonality = {
      id: AGENT_PERSONALITIES.CONVERSATION_PARTNER,
...
Remember: You're not a teacher, you're a conversation partner. Make it feel like chatting with a friend who happens to speak the language well.`
  ...
  private generateNaturalCorrections(content: string): string | null {
  ...
  private suggestNaturalExpressions(content: string): string | null {
  ...
  private detectNaturalLanguageUse(content: string): boolean {
  ...
  private generateNaturalLanguageEncouragement(content: string): string {
  ...
  private detectUnnaturalWordOrder(content: string): boolean {
  ...
  private makeResponseMoreConversational(content: string, context: ConversationContext): string {
  ...
  private addNaturalFollowUp(context: ConversationContext): string {
    const followUps = [
      " What about you?",
   ...
   */
  private generatePersonalAnecdote(topic: string): string {
