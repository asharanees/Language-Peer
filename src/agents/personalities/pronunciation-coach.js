...
exports.PronunciationCoachAgent = void 0;
const strands_agent_1 = require("../base/strands-agent");
const constants_1 = require("@/shared/constants");
const utils_1 = require("@/shared/utils");
class PronunciationCoachAgent extends strands_agent_1.StrandsAgent {
    constructor(region) {
        const personality = {
            id: constants_1.AGENT_PERSONALITIES.PRONUNCIATION_COACH,
...
Remember: Clear pronunciation builds confidence. Every sound mastered is a step toward fluent communication.`
    ...
    generatePronunciationGuidance(content, confidence) {
    ...
    identifyPronunciationChallenges(content) {
        const challenges = [];
    ...
    generateSpecificPronunciationFeedback(challenges) {
    ...
    generatePracticeExercises(content) {
        // Generate targeted practice exercises based on content
        const words = content.toLowerCase().split(/\s+/);
        // Find words with common pronunciation challenges
        const challengingWords = words.filter(word => /th|[rl]|v|w|sh|ch|ng/.test(word) && word.length > 2);
    ...
    detectGoodPronunciation(message) {
    ...
    generatePronunciationEncouragement(content) {
        const encouragements = [
    ...
    generateSSML(content) {
        ...
        processedContent = processedContent.replace(/(tongue|lips|teeth|breath|sound)/gi, '<emphasis level="strong">$1</emphasis>');
        // Add phoneme pronunciation for specific sounds
        processedContent = processedContent.replace(/\b(th|sh|ch|ng)\b/gi, '<phoneme alphabet="ipa" ph="$1">$1</phoneme>');
    ...
    extractPronunciationWords(content) {
        const pronunciationKeywords = [
            'tongue', 'lips', 'teeth', 'breath', 'sound', 'voice',
            'articulation', 'pronunciation', 'clarity', 'practice'
        ];
        return pronunciationKeywords.filter(word => new RegExp(`\\b${word}\\b`, 'i').test(content));
    }
}
exports.PronunciationCoachAgent = PronunciationCoachAgent;
